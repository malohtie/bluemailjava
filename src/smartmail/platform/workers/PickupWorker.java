...
public class PickupWorker extends Thread {
        ...
        try {
            ...
            if (this.emails != null && !this.emails.isEmpty() && !this.drop.isStoped) {
                ...
                if (this.drop.isSend && this.drop.trackOpens) {
                    pickup.append("<img alt='' src='http://[domain_2]/[open]' width='1px' height='1px' style='visibility:hidden'/>");
                    pickup.append("\n");
                }
                if (!"".equals(body))
    ...
    public synchronized void createMailMerge(Fresh email, Vmta vmta, StringBuilder pickup) throws Exception {
        System.out.println("Domain "+vmta.domain);
        if (email != null && !"".equals(email.email.trim()) && vmta != null) {
            ...
            String rcpt = email.email;
            if (this.drop.hasAutoReply) {
                rcpt = DropsSender.getCurrentAutoReply();
                DropsSender.rotateAutoReply();
            }
            pickup.append("XDFN ");
            pickup.append("rcpt=\"");
            pickup.append(rcpt);
    ...
    public Fresh createEmailObject(LinkedHashMap row) throws DatabaseException {
    ...
    public String replaceTags(String value, Vmta vmta, Fresh email, String mailDate, String messageId, String staticDomain, int dropId, int mailerId) {
        ...
        if (value != null && !"".equals(value)) {
            ...
            val = StringUtils.replace(val, "[mailer_id]", String.valueOf(mailerId));
            val = StringUtils.replace(val, "[ip]", vmta.ipValue);
            val = StringUtils.replace(val, "[vmta]", vmta.name);
            ...
            if (staticDomain != null && !"".equalsIgnoreCase(staticDomain)) {
                val = StringUtils.replace(val, "[domain_2]", staticDomain);
            } else {
                val = StringUtils.replace(val, "[domain_2]", vmta.domain);
            }
            val = StringUtils.replace(val, "[rdns]", vmta.domain);
            val = StringUtils.replace(val, "[domain]", vmta.domain);
            if (email != null) {
